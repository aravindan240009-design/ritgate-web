...
export default function StaffRequestTimeline({ request }: StaffRequestTimelineProps) {
  if (!request) return null;

  const { status, hodApproval, hodRemark } = request;
  ...
  const progressPercentage = (getCompletedStepsCount() / 2) * 100;
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center">
                <div className={cn("w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-transform", stepStatus === 'active' && "animate-pulse scale-105", config.bg)}>
              ...
              <div className="flex-1 pt-2">
                ...
                {/* Remarks */}
                {item.step === 2 && hodRemark && (
                  <div className="mt-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border-l-[3px] border-amber-500">
