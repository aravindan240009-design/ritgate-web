...
export function SkeletonCard() {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-[16px] border border-slate-100 dark:border-slate-800 p-4 mb-3 space-y-4 shadow-sm">
      {/* Top row: avatar + name block + time */}
      <div className="flex items-center gap-3">
        <Skeleton variant="circle" className="w-[48px] h-[48px] shrink-0" />
        <div className="flex-1 space-y-2">
          <Skeleton className="h-3.5 w-[60%] rounded-full" />
          <Skeleton className="h-2.5 w-[40%] rounded-full opacity-60" />
        </div>
        <Skeleton className="h-2.5 w-[40px] rounded-full self-start mt-1" />
      </div>

      {/* Info box */}
      <div className="bg-slate-50/50 dark:bg-slate-800/30 rounded-xl p-3.5 space-y-2.5">
        <Skeleton className="h-3 w-[90%] rounded-full" />
        <Skeleton className="h-3 w-[60%] rounded-full opacity-60" />
      </div>

      {/* Status pill */}
      <Skeleton className="h-[26px] w-[80px] rounded-[13px]" />
    </div>
  );
}
