...
const getInitials = (name: string) => (name || 'NA').split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
...
export default function AdminScanHistory({ onBack }: AdminScanHistoryProps = {}) {
  ...
  const loadGateLogs = useCallback(async (from?: string, to?: string) => {
    setLoading(true);
    try {
      const res = await getAdminGateLogs(from, to);
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      a.href = url; a.download = `Admin_Gate_Logs_${new Date().toISOString().slice(0, 10)}.csv`; a.click();
      URL.revokeObjectURL(url);
      showSuccess('Exported', 'Gate logs exported as CSV.');
    } catch { showError('Export Failed', 'Could not export.'); }
  };
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Gate Log History</h2>
                  ...
                  <Card>
                    <div className="flex items-center gap-3 mb-3">
                      <div className={cn('w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold shrink-0',
                        isEntry ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/30')}>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 space-y-2">
                      {log.purpose && log.purpose !== '-' && (
                        <div className="flex items-center gap-2"><FileText className="w-3.5 h-3.5 text-slate-400 shrink-0" /><span className="text-xs text-slate-600 dark:text-slate-300 truncate">{log.purpose}</span></div>
                      )}
